    ...
    public class SendoutProcessing
    {
            ...
            foreach (var sendout in sendouts)
            {
                try
                {
                    ...
                    var listOfContactDetails = meanOfCommunication == MeansOfCommunication.Email 
                        ? userList.Select(x => x.Email) 
                        : userList.Select(x => x.SMS);
        ...
        private MeansOfCommunication ExtractPriority(Template template, IEnumerable<User> user)
        {
            if (Convert.ToInt16(template.NotificationPriority) == (int) MeansOfCommunication.Email &&
               user.All(x => !String.IsNullOrEmpty(x.Email)))
                return MeansOfCommunication.Email;
            if (Convert.ToInt16(template.NotificationPriority) == (int) MeansOfCommunication.SMS &&
                user.All(x => !String.IsNullOrEmpty(x.SMS)))
                return MeansOfCommunication.SMS;
        ...
        {
            var parameterSet = sendout.Parameters.Split(",").Select(x => x.Trim().ToLower()).ToList();
            ...
            while (squareBracketsFound)
            {
                ...
                var wordWithBrackets = template.NotificationText.Substring(openBracketIndex, closeBracketIndex - openBracketIndex + 1).ToLower();
                template.NotificationText = dictionaryParam.ContainsKey(wordWithBrackets.Substring(1, wordWithBrackets.Length - 2)) ?
                     template.NotificationText.
                            Replace(wordWithBrackets,
                            dictionaryParam[wordWithBrackets.Substring(1, wordWithBrackets.Length - 2)]) :
                     template.NotificationText.
